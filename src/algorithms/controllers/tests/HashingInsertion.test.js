...
describe('HashingInsertion', () => {
  ...
  it('LP insert small table', () => {
    ...
    expect(HashingInsertion.run(chunker, { values: input.map(Number), hashSize: SMALL_SIZE, name: "HashingLP" })).toEqual(result);
  });
  ...
  it('LP insert large table', () => {
    ...
    expect(HashingInsertion.run(chunker, { values: input.map(Number), hashSize: LARGE_SIZE, name: "HashingLP" })).toEqual(result);
  });
  ...
  it('DH insert small table', () => {
    ...
    expect(HashingInsertion.run(chunker, { values: input.map(Number), hashSize: SMALL_SIZE, name: "HashingDH" })).toEqual(result);
  });
  ...
  it('DH insert large table', () => {
    ...
    expect(HashingInsertion.run(chunker, { values: input.map(Number), hashSize: LARGE_SIZE, name: "HashingDH" })).toEqual(result);
  });
});
