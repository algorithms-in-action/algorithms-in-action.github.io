...
\\Note{  REAL specification of 234 tree insertion and search
    ...
    \\Code{
    Insert 
    // insert key k into leaf node p (a two-node or three-node)
    if p is a two-node
    \\In{
    ...
    \\Code{
    Split 
    // Split four-node (c) into two two-nodes and move c.key2 to parent
    ...
    \\Code{
    InsertParent 
    // insert c1, c.key2 and c2 into node p (replacing c)
    if p = Empty
    \\In{
    ...
    \\Code{
    MoveToChild 
    // Move c to child of c that may contain key k
