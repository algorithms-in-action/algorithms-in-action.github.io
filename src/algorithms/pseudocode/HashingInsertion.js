...
const main = `
        ...
        \\Code{
            ...
            HashInsert(T, k)  // Insert key k into table
                \\In{
                    ...
                    \\Expl}
                    T[i] <- k // unoccupied slot found so we put k in it \\B 10
                    // Done \\B 11
                \\In}
            
            //=======================================================

            HashDelete(T, k)  // Delete key k in table T \\B 12
            \\In{
                i <- hash(k) // Expand Hash in HashInsert for details \\B 17
                Choose Increment value for stepping through T //Expand ChooseIncrement in HashInsert for details \\B 18
                while not (T[i] = k or T[i] = Empty or T[i] = "X") // search for k or Empty or Deleted \\B 13
                    \\In{
                        i <- (i + Increment) mod TableSize \\B 14
        ...
        \\Code{
            InsertionLoop
                ...
                \\Expl}
                while T[i] is occupied by another element // search for unoccupied slot \\B 8
                \\Expl{ If T[i] = k then k already exists in the table.  We could explicitly check
                        for this but the code here simply over-writes the previous
                        ocurrence of k, as if the slot was empty.
                \\Expl}
                \\In{
                    i <- (i + Increment) mod TableSize \\B 9
...
export const hash1 = `
        \\Code{
            Hash1
                i <- (k * BIGPRIME) mod TableSize \\B 6
...
export const doubleHashingIncrement = `
        ...
        \\Code{
            SetIncrement
                Increment <- (k * BIGPRIME2) mod SMALLISHPRIME + 1 \\B 7
